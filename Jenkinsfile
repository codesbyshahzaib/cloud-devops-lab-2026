    ...
    stages {
        ...
        stage('Lint') {
            steps {
                ...
                sh 'docker run --rm -v ${WORKSPACE}/app:/usr/src/app -w /usr/src/app node:18-alpine sh -c "npm install && npm run lint"'
            }
        }
        ...
        stage('Test') {
            steps {
                ...
                sh 'docker run --rm -v ${WORKSPACE}/app:/usr/src/app -w /usr/src/app node:18-alpine sh -c "npm run test"'
            }
        }
